...
int MySQLInstase::reconnect() {
  ...
  // DebugLog << "mysql fd is " << sql_handler_.net.fd;
  lock.unlock();
  if (!sql_handler_) {
  ...
  DebugLog << "begin to connect mysql{ip:" << option_.addr_.getIP() << ", port:" << option_.addr_.getPort()
  ...
  if (!mysql_real_connect(sql_handler_, option_.addr_.getIP().c_str(), option_.user_.c_str(), option_.passwd_.c_str(),
    ...
    ErrorLog << "faild to call mysql_real_connect, peer addr[ " << option_.addr_.getIP() << ":"
...
int MySQLInstase::query(const std::string &sql) {
  ...
  if (rt != 0) {
    ErrorLog << "excute mysql_real_query error, sql[" << sql << "], mysql sys errinfo[" << mysql_error(sql_handler_)
             << "]";
...
MYSQL_RES *MySQLInstase::storeResult() {
  ...
  if (count != 0) {
    ...
    if (!res) {
      ErrorLog << "excute mysql_store_result error, mysql sys errinfo[" << mysql_error(sql_handler_) << "]";
    } else {
