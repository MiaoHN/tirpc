...
class LoadBalanceStrategy {
 public:
  ...
  virtual Address::ptr select(std::vector<Address::ptr> &addrs, const TinyPbStruct &invocation) = 0;
};

// 随机策略
class RandomLoadBalanceStrategy : public LoadBalanceStrategy {
 public:
  Address::ptr select(std::vector<Address::ptr> &addrs, const TinyPbStruct &) {
...
class ConsistentHashLoadBalanceStrategy : public LoadBalanceStrategy {
 public:
  Address::ptr select(std::vector<Address::ptr> &addrs, const TinyPbStruct &invocation) {
    size_t identityHashCode = hashCode(addrs);
    std::lock_guard<std::mutex> lock(mutex_);  // C++的map不是线程安全的

    size_t index = invocation.service_full_name_.find('.');
    std::string serviceName = invocation.service_full_name_.substr(0, index);
    std::string methodName = invocation.service_full_name_.substr(index);
    ...
    return selector->select(methodName + invocation.pb_data_);  // 同一个方法和参数，请求会打到同一个虚拟节点上
  }

 private:
  class ConsistentHashSelector {
   public:
    ConsistentHashSelector(std::vector<Address::ptr> &invokers, int replicaNumber, int identityHashCode)
    ...
    std::map<size_t, Address::ptr> virtualInvokers;
    const size_t identityHashCode;

    static size_t hash(const std::vector<uint8_t> &digest, int number) {
      return (((size_t)(digest[3 + number * 4] & 0xFF) << 24) | ((size_t)(digest[2 + number * 4] & 0xFF) << 16) |
              ((size_t)(digest[1 + number * 4] & 0xFF) << 8) | (digest[number * 4] & 0xFF)) &
             0xFFFFFFFFL;
    }
    ...
    Address::ptr select(const std::string &rpcServiceKey) {
      std::vector<uint8_t> digest = md5(rpcServiceKey);
      return selectForKey(hash(digest, 0));
    }

    Address::ptr selectForKey(size_t hashCode) {
  ...
  // 计算vector的hashCode
  size_t hashCode(const std::vector<Address::ptr> &addrs) {
    size_t result = 1;
    for (auto &item : addrs) {
