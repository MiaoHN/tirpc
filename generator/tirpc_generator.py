...
class Generator:
    ...
    def gen_include_name(self):
        self.INCLUDE_PB_HEADER = (
            '#include "' + self.project_name + "/pb/" + self.project_name + '.pb.h"'
        )
        self.INCLUDE_SERVER_HEADER = (
            '#include "' + self.project_name + "/service/" + self.project_name + '.h"'
        )
        self.INCLUDE_INTERFACEBASE_HEADER = (
            '#include "' + self.project_name + '/interface/interface.h"'
        )
        self.INCLUDE_BUSINESS_EXCEPTION_HEADER = (
            '#include "' + self.project_name + '/comm/business_exception.h"'
        )
    ...
    def gen_exception(self):
        ...
        if not os.path.exists(exception_file):
            ...
            exception_template = Template(
                open(
                    self.generator_path + "/template/business_exception.h.template", "r"
                ).read()
            )
    ...
    def gen_server(self):
        ...
        head_file_temlate = Template(
            open(self.generator_path + "/template/server.h.template", "r").read()
        )
        head_file_content = head_file_temlate.safe_substitute(
            HEADER_DEFINE=project_name.upper()
            + "_SERVICE_"
            + project_name.upper()
            + "_H",
        ...
        cc_file_temlate = Template(
            open(self.generator_path + "/template/server.cc.template", "r").read()
        )
    ...
    def gen_main(self):
        ...
        if not os.path.exists(main_file):
            main_file_temlate = Template(
                open(self.generator_path + "/template/main.cc.template", "r").read()
            )
    ...
    def gen_interface_base(self):
        ...
        if not os.path.exists(header_file):
            file_temlate = Template(
                open(
                    self.generator_path + "/template/interface_base.h.template", "r"
                ).read()
            )
        ...
        if not os.path.exists(cc_file):
            file_temlate = Template(
                open(
                    self.generator_path + "/template/interface_base.cc.template", "r"
                ).read()
            )
            file_content = file_temlate.safe_substitute(
                FILE_NAME=self.project_name + ".h",
    ...
    def gen_each_interface(self):
        ...
        interface_test_client_file_template = Template(
            open(generator_path + "/template/test_tirpc_client.cc.template", "r").read()
        )
        ...
        for each in self.interface_list:
            ...
            if not os.path.exists(file):
                cc_file_content = interface_cc_file_temlate.safe_substitute(
                    PROJECT_NAME=project_name,
                    INCLUDE_PB_HEADER=self.INCLUDE_PB_HEADER,
                    INCLUDE_INTERFACE_HEADER='#include "'
                    + project_name
                    + "/interface/"
                    + each["interface_name"]
                    + '.h"',
            ...
            file = (
                self.test_client_path + "/test_" + each["interface_name"] + "_client.cc"
            )
            if not os.path.exists(file):
        ...
        print("End generate tirpc framework code")
        print("=" * 100)

    def generate_framework_code(self):
        print("=" * 100)
        print("Begin to generate tirpc framework code")
    ...
    def gen_conf_file(self):
        print("=" * 100)
        print("Begin to generate tirpc conf file")
        ...
        file.close()
        print("succ write to " + out_file)
        print("End generate tirpc conf file")
        print("=" * 100)
    ...
    def generate_tirpc_project(self):
        try:

            self.parseInput()

            print("=" * 150)
            print("Begin generate tirpc project")
            ...
            print("Succ generate tirpc project")
            print("=" * 150)

        except Exception as e:
            print("Falied generate tirpc project, err:")
            print(e)
            traceback.print_exc()
            print("=" * 150)
        finally:
    ...
    def printHelp(self):
        print("=" * 100)
        print("Welcome to use tirpc Generator, this is help document:\n")
        ...
        print("Usage:")
        print("tirpc_generator.py -[options][target]\n")
        print("Options:")
        print("-h, --help")
        ...
        print("")
        print("For example:")
        print("tirpc_generator.py -i order_server.proto -o ./")
    ...
    def parseInput(self):
        opts, args = getopt.getopt(
            sys.argv[1:], "hi:o:p:h:", ["help", "input=", "output="]
        )
        ...
        if not os.path.exists(self.proto_file):
            raise Exception(
                "Generate error, not exist protobuf file: " + self.proto_file
            )
...
if __name__ == "__main__":
    instanse = Generator()
    instanse.generate_tirpc_project()
